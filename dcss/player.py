...
class Player():
    ...
    def _update_health(self, terminal):
        ...
        health = terminal.get_text(37,2,18,1).split(':')[1].strip().split('/')
    ...
    def _update_evasion(self, terminal):
        ...
        self.evasion = int(terminal.get_text(37,5,18,1).split(':')[1].strip())

    def _update_shielding(self, terminal):
        #grab SH from sidebar
        self.shielding = int(terminal.get_text(37,6,18,1).split(':')[1].strip())
    ...
    def _update_experience_level(self, terminal):
        self.experience_level = int(terminal.get_text(37,7,7,1).split(':')[1]
                .strip())

    def _update_next_experience_level(self, terminal):
        #make sure to remove '%' character before conversion
        self.next_experience_level = int(terminal.get_text(44,7,11,1).split(':')[1]
                .strip()[:-1])
